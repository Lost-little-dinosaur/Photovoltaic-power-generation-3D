...
def getColumnsInformation():
    ...
    column = {  # 0表示规则 1表示拼接 1上竖排 2横排 3下竖排
        ...
        ("182-78膨胀常规", 3, 0, 1, 0, 0, 0): [],
        ("182-78膨胀常规", 3, 0, 1, 0, 0, 1): [],

        ("182-78膨胀常规", 3, 0, 0, 0, 1, 0): [],
        ("182-78膨胀常规", 3, 0, 0, 0, 1, 1): [],

        ("182-78膨胀常规", 3, 1, 1, 0, 0, 0): [],
        ("182-78膨胀常规", 3, 1, 1, 0, 0, 1): [],

        ("182-78膨胀常规", 3, 1, 2, 0, 0, 0): [],
        ("182-78膨胀常规", 3, 1, 2, 0, 0, 1): [],

        ("182-78膨胀常规", 3, 1, 0, 1, 0, 0): [],
        ("182-78膨胀常规", 3, 1, 0, 1, 0, 1): [],

        ("182-78膨胀常规", 3, 1, 0, 0, 1, 0): [],
        ("182-78膨胀常规", 3, 1, 0, 0, 1, 1): [],

        ("182-78膨胀常规", 4, 0, 1, 0, 0, 0): [],
        ("182-78膨胀常规", 4, 0, 1, 0, 0, 1): [],

        ("182-78膨胀常规", 4, 0, 2, 0, 0, 0): [],
        ("182-78膨胀常规", 4, 0, 2, 0, 0, 1): [],

        ("182-78膨胀常规", 4, 0, 0, 0, 1, 0): [],
        ("182-78膨胀常规", 4, 0, 0, 0, 1, 1): [],

        ("182-78膨胀常规", 4, 0, 0, 0, 2, 0): [],
        ("182-78膨胀常规", 4, 0, 0, 0, 2, 1): [],

        ("182-78膨胀常规", 4, 1, 1, 0, 0, 0): [],
        ("182-78膨胀常规", 4, 1, 1, 0, 0, 1): [],

        ("182-78膨胀常规", 4, 1, 2, 0, 0, 0): [],
        ("182-78膨胀常规", 4, 1, 2, 0, 0, 1): [],

        ("182-78膨胀常规", 4, 1, 3, 0, 0, 0): [],
        ("182-78膨胀常规", 4, 1, 3, 0, 0, 1): [],

        ("182-78膨胀常规", 4, 1, 0, 1, 2, 0): [],  # 可能出错
        ("182-78膨胀常规", 4, 1, 0, 1, 2, 1): [],

        ("182-78膨胀常规", 4, 1, 0, 1, 1, 0): [],
        ("182-78膨胀常规", 4, 1, 0, 1, 1, 1): [],

        ("182-78膨胀常规", 4, 1, 0, 0, 1, 0): [],
        ("182-78膨胀常规", 4, 1, 0, 0, 1, 1): [],

        ("182-78膨胀常规", 5, 0, 1, 0, 0, 0): [],  # 0表示左边部分 1表示右边部分
        ("182-78膨胀常规", 5, 0, 1, 0, 0, 1): [],

        ("182-78膨胀常规", 5, 0, 2, 0, 0, 0): [],
        ("182-78膨胀常规", 5, 0, 2, 0, 0, 1): [],

        ("182-78膨胀常规", 5, 0, 3, 0, 0, 0): [],
        ("182-78膨胀常规", 5, 0, 3, 0, 0, 1): [],

        ("182-78膨胀常规", 5, 0, 0, 0, 1, 0): [],
        ("182-78膨胀常规", 5, 0, 0, 0, 1, 1): [],

        ("182-78膨胀常规", 5, 0, 0, 0, 2, 0): [],
        ("182-78膨胀常规", 5, 0, 0, 0, 2, 1): [],

        ("182-78膨胀常规", 5, 0, 0, 0, 3, 0): [],
        ("182-78膨胀常规", 5, 0, 0, 0, 3, 1): [],
    }  # 182-78常规
    limit_column = {
    ...
    arrangement_height = {  # 0表示规则 1表示拼接 1上竖排 2横排 3下竖排
        ...
        ("182-78膨胀常规", 3, 0, 1, 0, 0): [],

        ("182-78膨胀常规", 3, 0, 0, 0, 1): [],

        ("182-78膨胀常规", 3, 1, 1, 0, 0): [],

        ("182-78膨胀常规", 3, 1, 2, 0, 0): [],

        ("182-78膨胀常规", 3, 1, 0, 1, 0): [],

        ("182-78膨胀常规", 3, 1, 0, 0, 1): [],

        ("182-78膨胀常规", 4, 0, 1, 0, 0): [],

        ("182-78膨胀常规", 4, 0, 2, 0, 0): [],

        ("182-78膨胀常规", 4, 0, 0, 0, 1): [],

        ("182-78膨胀常规", 4, 0, 0, 0, 2): [],

        ("182-78膨胀常规", 4, 1, 1, 0, 0): [],

        ("182-78膨胀常规", 4, 1, 2, 0, 0): [],

        ("182-78膨胀常规", 4, 1, 3, 0, 0): [],

        ("182-78膨胀常规", 4, 1, 0, 1, 2): [],  # 可能出错

        ("182-78膨胀常规", 4, 1, 0, 1, 1): [],

        ("182-78膨胀常规", 4, 1, 0, 0, 1): [],

        ("182-78膨胀常规", 5, 0, 1, 0, 0): [],  # 0表示左边部分 1表示右边部分

        ("182-78膨胀常规", 5, 0, 2, 0, 0, 0): [],

        ("182-78膨胀常规", 5, 0, 3, 0, 0): [],

        ("182-78膨胀常规", 5, 0, 0, 0, 1): [],

        ("182-78膨胀常规", 5, 0, 0, 0, 2): [],

        ("182-78膨胀常规", 5, 0, 0, 0, 3): [],
    }
