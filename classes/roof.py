...
class Roof:
    ...
    def calculateShadow(self):
        self.obstacleArray = np.full((self.length, self.width), True)
        time1 = time.time()
        ...
        for eachObstacle in self.obstacles:
            ...
            for x1 in range(round(minX), round(maxX) + 1):  # todo: 时间太长了，需要优化！
                for y1 in range(round(minY), round(maxY) + 1):
                    self.obstacleArray[x1, y1] = not isPointInsideConvexHull(hullPoints, x1, y1)
                    ...
                    self.showArray[x1, y1] = EmptyColor if self.obstacleArray[x1, y1] else ShadowColor
        # numpy优化
        self.boolArraySum = np.cumsum(np.cumsum(self.obstacleArray.astype(int), axis=0), axis=1)
